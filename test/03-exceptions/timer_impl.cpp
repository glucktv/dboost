...
long timer_impl::add_timer(const long timeout_ms) throw (dboost_test::wrong_timeout_exception)
{  
    clog << __FUNCTION__ << " to = " << timeout_ms << " id = " << m_last_id << endl;
    m_timers[m_last_id] = timeout_ms;
    throw dboost_test::wrong_timeout_exception("Testing wrong timeouts");
    return m_last_id++;
}
