...
#define _dbus_verbose(x...) fprintf(stderr, x)
                             ...
                             void *user_data)
{
  ...
  if (type == DBUS_MESSAGE_TYPE_METHOD_RETURN)
  ...
  else if (type == DBUS_MESSAGE_TYPE_ERROR)
    {
     ...
     assert("Unexpected message received\n");
             ...
             DBusMessage        *message)
{
  ...
  char* doubled = malloc(strlen(s) * 2 + 1);
  strcpy(doubled, s);
  strcpy(doubled + strlen(s), s);
  
  if (!dbus_message_append_args (reply,
                                 DBUS_TYPE_STRING, &doubled,
                                 DBUS_TYPE_INVALID))
    die ("No memory");
